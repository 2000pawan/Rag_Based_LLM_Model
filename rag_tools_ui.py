...
# -- 1️⃣ Model and api_key-------------------------------------------------------------------------------

embedding_model = HuggingFaceEmbeddings(model_name = "sentence-transformers/all-MiniLM-L6-v2")
llm = ChatOllama(model="llama3.1",temperature=0.8)
# api_key = "AIzaSyA24zpeAeNBDyzniu_2oSIdydR41dPJg6U"
...
def create_agent(retriever):
    ...
    response=create_react_agent(
        model=llm,
        tools=[rag_tool],
        prompt="you are a helpfull assistant . if user say hi hello with name or with out name then greet them politilly response and if user query another query for general questions then use the `rag_tool `Answer ONLY from the provided PDF context, using `rag_tool` to pull passages.If the context is insufficient, if you did not find answer of question fron the `rag_tool` then say just say you don't know.",
        checkpointer=memory,
    )
    return response
